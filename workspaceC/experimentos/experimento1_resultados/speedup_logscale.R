...
plot(xn, -log(-yn), axes = F, xlim = range(1:(length(yn)+length(yp))), ylim = c(-logyrange,logyrange), type = 'h', lty = 3, ylab = "", xlab = "Experimentos ordenados por ganho de desempenho", main = "")
...
title(mgp = c(4,1,0), ylab = "Ganho de desempenho")
...
axis(side = 1, pos = 0, xaxp = c(1,length(c(xn,xp)),20), labels = F, tcl = -0.25)
axis(side = 2, las = 2, at = seq(-logyrange,logyrange,length.out=7), labels = paste(c(negloglabel,posloglabel),"x"))
